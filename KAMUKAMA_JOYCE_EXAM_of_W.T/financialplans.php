...
<head>
  ...
  <title>financialplans Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO financialplans(PlanID, , PlannerID,ClientID,Title,Description) VALUES (?, ?, ?,?,?)");
    $stmt->bind_param("issss", $PlanID, $PlannerID,$ClientID,$Title,$Description);
    // Set parameters and execute
    $PlanID = $_POST['book_id'];
    $PlannerID = $_POST['ride_id'];
    $ClientID = $_POST['passenger_id'];
    $Title = $_POST['passengerid'];
    $Description = $_POST['passengeid'];
...
<body>
    <center><h2>financialplans Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all financialplans
$sql = "SELECT * FROM financialplans";
$result = $connection->query($sql);

// Check if there are any financialplans
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $PlanID = $row['PlanID']; // Fetch the PlanID
        echo "<tr>
            <td>" . $row['PlanID'] . "</td>
            <td>" . $row['PlannerID'] . "</td>
            <td>" . $row['ClientID'] . "</td>
            ...
            <td><a style='padding:4px' href='delete_financialplans.php?PlanID=$PlanID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_financialplans.php?PlanID=$PlanID'>Update</a></td> 
        </tr>";
    }
