...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <label for="GoalText">GoalText:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO goals(GoalID, , UserID,GoalText,Deadline) VALUES (?, ?, ?,?)");
    $stmt->bind_param("isss", $GoalID, $UserID,$GoalText,$Deadline);
    
    // Set parameters and execute
    $GoalID = $_POST['book_id'];
    $UserID = $_POST['ride_id'];
    $GoalText = $_POST['passenger_id'];
    $Deadline = $_POST['passengerid'];
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $GoalID = $row['GoalID']; // Fetch the GoalID
        echo "<tr>
            <td>" . $row['GoalID'] . "</td>
            <td>" . $row['UserID'] . "</td>
            <td>" . $row['GoalText'] . "</td>
            <td>" . $row['Deadline'] . "</td>
            <td><a style='padding:4px' href='delete_goals.php?GoalID=$GoalID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_goals.php?GoalID=$GoalID'>Update</a></td> 
        </tr>";
    }
