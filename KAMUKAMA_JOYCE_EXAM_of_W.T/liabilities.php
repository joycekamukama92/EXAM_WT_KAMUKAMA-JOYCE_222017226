...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO liabilities(LiabilityID,UserID , LiabilityName,Amount) VALUES (?, ?, ?,?)");
    $stmt->bind_param("isss", $LiabilityID, $UserID,$LiabilityName,$Amount);
    
    // Set parameters and execute
    $LiabilityID = $_POST['book_id'];
    $UserID = $_POST['ride_id'];
    $LiabilityName = $_POST['passenger_id'];
...
<body>
    <center><h2>liabilities Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all liabilities
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $LiabilityID = $row['LiabilityID']; // Fetch the LiabilityID
        echo "<tr>
            <td>" . $row['LiabilityID'] . "</td>
            <td>" . $row['UserID'] . "</td>
            <td>" . $row['LiabilityName'] . "</td>
            <td>" . $row['Amount'] . "</td>
            <td><a style='padding:4px' href='delete_financialplans.php?LiabilityID=$LiabilityID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_financialplans.php?LiabilityID=$LiabilityID'>Update</a></td> 
        </tr>";
    }
