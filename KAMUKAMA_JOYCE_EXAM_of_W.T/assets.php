...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO assets(AssetID, UserID, AssetName,Value) VALUES (?, ?, ?,?)");
    $stmt->bind_param("isss", $AssetID, $UserID, $AssetName,$Value);
    // Set parameters and execute
    $AssetID = $_POST['book_id'];
    $UserID = $_POST['ride_id'];
    $AssetName = $_POST['passenger_id'];
    $Value = $_POST['passengerid'];
...
<body>
    <center><h2>Assets Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all assets
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $AssetID = $row['AssetID']; // Fetch the AssetID
        echo "<tr>
            ...
            <td>" . $row['AssetName'] . "</td>
            <td>" . $row['Value'] . "</td>
            <td><a style='padding:4px' href='delete_assets.php?AssetID=$AssetID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_assets.php?AssetID=$AssetID'>Update</a></td> 
        </tr>";
    }
