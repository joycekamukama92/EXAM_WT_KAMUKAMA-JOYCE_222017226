...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO planners(PlannerID,UserID) VALUES (?, ?)");
    $stmt->bind_param("is", $PlannerID, $UserID);
   
    // Set parameters and execute
    $PlannerID = $_POST['book_id'];
...
<body>
    <center><h2>planners Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all planners
$sql = "SELECT * FROM planners";
$result = $connection->query($sql);

// Check if there are any planners
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $PlannerID = $row['PlannerID']; // Fetch the PlannerID
        echo "<tr>

            <td>" . $row['PlannerID'] . "</td>
            <td>" . $row['UserID'] . "</td>
            <td><a style='padding:4px' href='delete_planners.php?PlannerID=$PlannerID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_planners.php?PlannerID=$PlannerID'>Update</a></td> 
        </tr>";
    }
